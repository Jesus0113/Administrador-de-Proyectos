...
export class TaskController {
    ...
    static getTaskById = async (req:Request, res: Response) => {
        ...
        try {
            const task = await (await (await Task.findById(req.task.id)).populate({path: 'completedBy.user', select: 'id name email'})).populate({path: 'notes', populate: {path: 'createdBy',  select: 'id name email'}})
            res.json(task);
        } catch (error) {
            res.status(500).json({error: 'Hubo un error'})
        }
